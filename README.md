...
The ResNet model sounds like an adequate model for detecting cars as it is trained on ImageNet that conatins 14 million of labeled images spanning more than 20,000 categories. However, when the same model is used to detect cars in destorted images or in different environemnt (in a jungle rather than on a road) the model is unable to detect cars correctly. In this situations the model explainers can be used to understand why the model is failing. For further details you can refer to the full colab notebook of my project ().
